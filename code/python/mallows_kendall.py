...
def fit_mm(rankings, s0=None):
    ...
    try:
        theta = sp_opt.newton(mle_theta_mm_f, 0.01, fprime=mle_theta_mm_fdev, args=(n, dist_avg), tol=1.48e-08,
                              maxiter=500, fprime2=None)
    except:
        if dist_avg == 0.0:
...
def expected_v(n, theta=None, phi=None, k=None):  # txapu integrar
    ...
    expected_v = np.exp(-theta[rnge]) / (1 - np.exp(-theta[rnge])) - (n - rnge) * np.exp(-(n - rnge) * theta[rnge]) / (
...
def variance_v(n, theta=None, phi=None, k=None):
    ...
    if type(phi) != list:
        phi = np.full(k, phi)
    rnge = np.array(range(k))
    var_v = phi[rnge] / (1 - phi[rnge]) ** 2 - (n - rnge) ** 2 * phi[rnge] ** (n - rnge) / (
                1 - phi[rnge] ** (n - rnge)) ** 2
    return var_v
...
def fit_gmm(rankings, s0=None):
    ...
    try:
        theta = []
        for j in range(1, n):
            theta_j = sp_opt.newton(mle_theta_j_gmm_f, 0.01, fprime=mle_theta_j_gmm_fdev, args=(n, j, V_avg[j - 1]),
                                    tol=1.48e-08, maxiter=500, fprime2=None)
...
def mle_theta_mm_fdev(theta, n, dist_avg):
    ...
    rnge = np.array(range(1, n))
    aux = np.sum(
        (n - rnge + 1) * (n - rnge + 1) * np.exp(-theta * (n - rnge + 1)) / pow((1 - np.exp(-theta * (n - rnge + 1))),
                                                                                2))
